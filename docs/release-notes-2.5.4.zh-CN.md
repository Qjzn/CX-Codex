...
GitHub Release 工作流和六个公开资产已通过。正式归档完成真实五进程卸载，结果 `warnings` 为空；安装目录、回滚目录、启动器、PID 标记、7420 监听和受管通道均已清理，用户配置与 Codex 登录态保持不变。卸载器只处理通过安装路径、配置、PID 标记和监听端口验证的受管进程；不会扫描或删除用户工作区、Codex 登录材料和 Android 签名文件。
