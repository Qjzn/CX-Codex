...
2.5.3 已正式发布，并从公开资产完成“运行中升级 → 同版本重复升级 → 默认保留数据卸载 → 无安装状态重装”全生命周期回归。升级目录锁已关闭；正式回归发现的卸载超时误警告已在 2.5.4 候选中改为子进程优先停止整棵受管进程树。协议基线漂移与历史 Google/Firebase key 仍需按下述边界持续处置。
...
4. 2.5.4 Release 发布后，从正式归档再次执行真实五进程卸载，要求 `warnings` 为空，再恢复远程服务。
