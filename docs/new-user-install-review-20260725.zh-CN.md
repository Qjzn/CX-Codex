...
首次源码预览安装最终成功用时 153.5 秒。正式 2.5.6 已完成公开资产与安装回归，并补上仅限本机的二维码配对页。2.5.7 候选进一步修复外层 PowerShell 变量或自动化管道捕获安装 JSON 时，后台服务已启动但调用脚本不退出的问题。项目仍不能称为真正“傻瓜式”，主要差距变为：下载阶段缺少连续进度、首次工作区选择不够明确，以及手机扫码后仍需手动输入密码。
...
| PowerShell 捕获安装 JSON | 2.5.7 候选已在外层变量捕获、真实服务健康、三项公网鉴权和官方卸载链路中通过 |
...
- 正式 2.5.6 已提供仅限本机的二维码配对页；2.5.7 候选已关闭 PowerShell 捕获安装结果时的后台服务句柄挂起。
