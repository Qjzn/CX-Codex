...
- Android 语音输入：Android 壳在 `http://` 远程连接下改为优先调用系统语音识别，不再受 WebView `getUserMedia` 的安全上下文限制；首次使用只申请麦克风权限，识别文字默认回填输入框供编辑确认。普通网页仍保留 HTTPS 录音转写，HTTP 网页可使用系统录音/音频上传回退。
