...
## Findings: Android Renderer Recovery and Single-message Ownership (2026-08-05)

- Installed Windows Codex `26.727.6591` keeps a static startup loader in `webview/index.html`, so visible recovery exists before the JavaScript application mounts. CX-Codex already has an equivalent boot shell; its reported background-return white screen instead came from a missing native WebView renderer recovery boundary.
- One reported duplicate prompt mapped to exactly one durable Runtime request and one App Server turn. The second bubble was frontend-only: legacy outbox restoration recomputed its signature baseline after authoritative history already contained the prompt.
...
## Findings: Spectator Reads and Explicit Writer Standby (2026-08-28)

- Installed Windows Codex `26.818.5229` presents writer collision as a dedicated task-level state: `已在另一个应用中打开`, a concise close-the-other-task explanation, and one `重试` action. It does not present the collision as an ordinary failed prompt.
- The current App Server contract separates `thread/read` from `thread/resume`. Reading is observational, while resume reopens a task for later turns and can claim the single writer; CX-Codex must preserve that boundary in every fallback and recovery path.
- CX-Codex adapts the official single-host rule in two safe modes. Spectator reads use bounded cache/session-log recovery without resume, while sends encountered under another writer retain stable identity and delegate to the native thread queue.
- Immediate official-desktop takeover remains an explicit operational transition, not fake multi-writer support. A persistent standby marker makes `service stop` survive the watchdog and login launcher; `service start` or `restart` is the reversible exit.
- Independent Headless Playwright at `393 x 852` verified the native-writer handoff surface with two ordered rows, zero retry actions, two delete actions, no false loading state, no browser errors, and no horizontal overflow. The captured viewport keeps the queue explanation and both rows visible for review.

## Findings: Sema-based Turn Transcript Rebuild (2026-08-30)

- Installed Windows Codex `26.818.5229` remains the interaction and safety cross-check: user prompts, assistant work, pending approvals, and final output retain distinct ownership with concise status and accessible actions. It is not the source of CX-Codex session semantics.
- `midea-ai/sema-code-core` at `f564e8d930053becdd5c31fe53f65fd863b6f283` is the mandatory semantic baseline for event projection, turn grouping, process activity, file changes, timing, and completion. CX-Codex intentionally tightens final selection to exact `phase: final_answer`; phase-less or commentary assistant text cannot become final by position.
- The rebuilt conversation surface consumes one pure `ConversationProjection`, keeps active process content expanded and completed process content collapsed, presents file changes and pending requests inside the owning turn, and has no legacy tail overlay or `Worked for`/command-sum timing owner.
- Independent CDP-driven Chrome and Headless Playwright verification covered `1440 x 900`, `393 x 852`, and coarse-pointer `884 x 1104`: one explicit final, one file summary, two request cards, zero legacy overlays, and zero horizontal overflow. Phone and foldable disclosure/action targets have a 44 px minimum; the foldable check caught 19-36 px controls before the coarse-pointer rule was added. Screenshots are under `output/regression-7420/conversation-sema-rebuild-20260830`.
- The 1600-event mobile stress fixture retained all 1600 projected activities while mounting 18 activity rows. Five isolated streaming runs completed 69-70 updates with 66-67 heartbeats, no Long Task at or above the browser's 50 ms reporting threshold (`maxLongTaskMs = 0`), no horizontal overflow, and a responsive action; separate timer-queue lag was 88-125 ms and is not treated as a single main-thread block. Auxiliary cold-navigation traces recorded 123-207 ms Long Tasks dominated by whole-document initial Layout even with zero stress items. That diagnostic is not the 1600-message streaming `<80 ms` gate and does not replace the separate real `local-cache` first-screen `<=300 ms` measurement.
- A spectator App Server may return `thread not loaded` even though the exact rollout JSONL exists. Read-only parity requires resolving that UUID under `sessions` or `archived_sessions` and projecting the log without `thread/resume`; writer takeover is not a rendering fallback.
- Hidden internal continuation user items carry authoritative `turn_id` boundaries even though their text must never render. Preserving that boundary keeps later commentary, file changes, and exact `final_answer` items in their own turns. `patch_apply_end.changes` maps to bounded `fileChange` items, while event/response message mirrors dedupe only when phase and normalized text both match; distinct stage messages remain visible process evidence.
- An isolated real-session replay on desktop, `393 x 852` phone, and coarse-pointer `884 x 1104` foldable recovered 8 turns, 6 historical finals with at most one final per turn, and 4 file-summary groups, with no internal-context leak, legacy overlay, or horizontal overflow. Phone and foldable standalone controls measured at least 44px high. Expected raw thread/goal RPC 502s, disabled cached reload, and the still-active final turn remain isolation limits, so this is not production-runtime or device evidence. Screenshots are under `output/regression-7420/conversation-sema-real-runtime-20260830`.

## Findings: Sema-aligned Composer Shell (2026-08-30)

- The mandatory Sema reference at `f564e8d930053becdd5c31fe53f65fd863b6f283` uses one `max-w-3xl` Composer shell, a light `rounded-xl` border, a 52px input layer, and a 44px control layer. The installed Codex desktop remains the interaction/safety cross-check for multiline input, IME ownership, pending work, and accessible actions; it does not override the Sema visual axis.
- CX-Codex keeps its existing attachment, model/quality/speed, long-input, dictation, send/stop, queue, and mobile-modal ownership. Only the visible shell alignment and surface weight changed: the border aligns to the `48rem` transcript column, uses a 12px radius with no resting shadow, and adds a one-pixel focus ring.
- Isolated Chrome verification measured a 768px visible shell at `1440 x 900` and coarse-pointer `884 x 1104`, 12px radius, no resting shadow, zero horizontal overflow, 36px/32px minimum desktop control dimensions, and 44px minimum phone/foldable targets. Screenshots are under `output/regression-7420/conversation-sema-composer-20260830`.

## Findings: Projected Pending-interaction Ownership (2026-08-30)

- Installed Windows Codex `26.818.5229` keeps pending approval and user-input requests owned by the task interaction layer, with concise typed surfaces and stable response identity. CX-Codex uses this only as an interaction/safety cross-check; the fixed Sema commit remains the transcript semantic source.
- CX-Codex now projects approval, user-input, MCP input/authorization, MCP approval, unsupported-tool, and generic requests into the owning turn with display type, title, detail, questions, safe HTTP(S) authorization URL, numeric response ID, and session-allow capability.
- `ThreadConversation.vue` no longer receives `UiServerRequest` or classifies raw request methods/params. It renders projected interaction blocks and emits only approve, decline, or answer intent; execution remains owned by the existing App Server/Runtime request responder.
- A pending interaction is authoritative for the visible waiting state even if a stale Runtime snapshot still reports `running`, preventing waiting time from being counted as active execution. Static gates reject reintroduction of raw request parsing in the conversation renderer.
- MCP persistence remains capability-driven: the projection exposes only `session` / `always` scopes advertised by `_meta.persist`, approval cards show bounded tool identity and target parameters, and replies preserve the Codex App payload shape. URL authorization omits unrelated freeform input, while user-input requests require an explicit answer before submission.

## Findings: Sema-aligned Quiet Shell (2026-08-30)

- The fixed Sema commit uses a 256px default resizable sidebar, a 44px ChatView header, and a `max-w-3xl` reading column. CX-Codex maps that hierarchy to a 288px default sidebar for Chinese task labels, clamps saved widths to 240-360px, and keeps the 44px/48rem header and Composer axis.
- The previous width reader coerced missing storage through `Number(null) === 0`, so a user with no preference received the minimum instead of the documented default. Missing or blank storage must return the default before numeric parsing; explicit values such as 340px remain preserved.
- The content grid owns the Composer's internal desktop gutters, leaving its visible border aligned to the same 768px axis as the Header. Status text can widen without moving the title or main axis.
- Installed Windows Codex `26.818.5229` remains the interaction/safety cross-check for status ownership and accessible names; the fixed Sema source remains the visual hierarchy reference. CDP checks covered light/dark 1440 x 900, 393 x 852, and 852 x 393 plus a saved-width case and status-text transition; independent Headless Playwright captured desktop and phone screenshots under `output/regression-7420/quiet-shell-playwright-20260830`.

## Findings: Sema-aligned Sidebar Hierarchy (2026-08-30)

- The fixed Sema source keeps task creation visually primary and project/history navigation quiet. CX-Codex maps that hierarchy to one full-width `新会话` row followed by search, workspace, and tools in a compact three-column row; the existing events, menus, project ordering, pinning, collapse state, and search state machine remain authoritative.
- Pinned rows are an intentional shortcut, not a new task collection: one pinned task may appear once in the pinned section and once in its project. Waiting tasks retain explicit text, and low-value counts remain tertiary text rather than pills.
- Background project reordering exposed a top-boundary bug: when the first project began exactly at the visible project region, `visibleTopInGroups <= 0` discarded the anchor. Clamping the offset to zero preserves the first visible project; the regression fixture measured the same 7.046875px viewport offset before and after promotion.
- CDP checks cover default and saved sidebar widths, light/dark contrast, mobile touch rows, menu fit, stale-search merge, query continuity, project reorder anchoring, current-thread reveal, and zero horizontal overflow. Independent Headless Playwright captured the desktop candidate under `output/regression-7420/quiet-sidebar-playwright-20260830`.

## Findings: Sema-aligned Composer Interaction and Queue Reorder (2026-08-30)

- The fixed Sema commit remains the Composer visual source: one quiet `max-w-3xl` work surface with restrained border, radius, spacing, and motion. Installed Windows Codex `26.818.5229` remains the interaction/safety cross-check for multiline input, IME handling, edit/delete/reorder/Steer queue actions, and accessible pending-work controls.
- CX-Codex keeps the existing draft, outbox, attachment, runtime send, durable queue, writer collision, and first-failure pause owners. The Composer shell changes only presentation; queue reorder adds explicit up/down controls only when every row is durably persisted, processing is idle, the first row is not failed, and all rows share one ownership kind.
- Reordering writes the stable local identity order before reusing the existing durable queue handoff and server reconciliation. Native-writer and 7420 queue reorder continue through their established APIs, including authoritative resync after a server order conflict; no second queue state, drag dependency, timer, or optimistic execution path was introduced.
- Isolated Chrome verification covered desktop, `393 x 852` phone, and coarse-pointer `884 x 1104`: bounded auto-grow, desktop/phone Enter semantics, IME key 229, editable dictation, attachment/runtime panel focus ownership, no overlap or horizontal overflow, and local visible feedback below 13 ms. The phone queue completed down/reverse and up/restore with 44 x 44 controls. Evidence is under `output/regression-7420/quiet-composer-audit-20260830`.

## Findings: Sema-aligned Responsive and Accessibility Hardening (2026-08-30)

- The fixed Sema commit remains the visual hierarchy reference, while installed Windows Codex `26.818.5229` remains the interaction and accessibility cross-check. Hardening must preserve the quiet shell instead of adding accessibility-only chrome, cards, navigation, or state owners.
- Browser coverage spans `1440 x 900`, coarse-pointer `884 x 1104`, `768 x 1024`, `393 x 852`, and `852 x 393` in light and dark themes. Each case checks horizontal overflow, named landmarks/navigation/controls, hidden hit targets, duplicate IDs, text-backed status, and selected WCAG AA contrast samples.
- A narrow visible resize rail can retain a 44px coarse-pointer hit area through an invisible pseudo-element only when the rail owns an explicit stacking position. Forced-colors verification must use authentic keyboard focus; programmatic focus alone does not prove the Tab route or visible system outline.
- Mobile drawer parity requires one named modal dialog, background `inert`, root scroll ownership, forward and reverse focus wrapping, inability to focus the real background opener, release on close, and focus return. A synthetic node appended after the background was frozen is not valid evidence about the owned page tree.
- Reduced-motion verification confirms that no visible infinite animation remains. Browser media emulation and screenshots are local candidate evidence only; they do not replace physical Windows high-contrast, system screen-reader, Android, or foldable-device checks.

## Findings: Projected-turn Virtualization and Scroll Ownership (2026-08-30)

- Installed Windows Codex `26.818.5229` isolates long-thread layout in `thread-virtualizer-D5BSbZuA.js`. Entries are keyed by `turnKey`, default to a 280px estimate, accept measured heights, retain explicit gaps, and calculate a bounded range from viewport height, distance from the bottom, and an overscan count.
- The virtualizer exposes anchor-preserving range and distance corrections by stable turn key. The matching `thread-scroll-layout-4ls8_8Uq.js` keeps a 24px bottom-follow threshold, a single distance-from-bottom owner, resize compensation, and `overflow-anchor: none` on the scroll container.
- CX-Codex adapts that infrastructure boundary to its top-origin Vue transcript while keeping the fixed Sema projection as semantic authority: at most 10 projected turns / 20 message nodes mount, measured height changes preserve the visible turn or live tail, and stale follow-bottom work must yield to newer user scrolling. No legacy `UiMessage`, final, timing, folding, or overlay rule participates in virtualization.

## Findings: Projected Session-log Terminal Convergence (2026-08-30)

- The fixed Sema projection and installed Codex host agree on the ownership boundary: turn lifecycle state is authoritative for completion, while the last visible assistant message is not. An explicit final remains a separate projected fact and may legitimately be absent after failure, interruption, or stop.
- CX-Codex still had one legacy recovery predicate outside the renderer: the quiet-window session-log scheduler inferred terminal evidence from the last normalized assistant message and its non-commentary phase. That could trigger or defer an authoritative history refresh for the wrong reason even though visible final selection was already exact.
- Session-log convergence now reads the latest turn state from the same `ConversationProjection` used by task presentation. Only `completed`, `failed`, `interrupted`, or `stopped` settles the quiet-window gate; queued, running, waiting, and sync-degraded turns remain deferred.
- The same audit found that stop availability and stale-command recovery still treated assistant text after a running command or latest user message as completion. Those recovery paths now consume the shared latest projected-turn terminal predicate as well; a visible assistant fragment cannot settle execution, while pending interactions and fresh active Runtime state remain non-terminal.
- Frontend normalizer tests cover every projected terminal and non-terminal state, while the source gate rejects reintroduction of `latestAssistant`, `message.phase`, either removed assistant-order helper, or the old message-evidence helper. Runtime protocol/store, queue, outbox, approval execution, notification protocol, and rendering remain unchanged.

## Findings: Sema-aligned Collaboration Activities (2026-08-30)

- The fixed Sema source treats agent work as first-class turn activity with human-readable instructions. Installed Windows Codex `26.818.5229` remains only the interaction and safety cross-check; it does not replace Sema as the transcript semantic authority.
- CX-Codex previously projected every `collabAgentToolCall` as the same “协同子任务” label and exposed the raw protocol enum as its target. The result was structurally present but not meaningfully scannable.
- The single `ConversationProjection` now maps `spawnAgent`, `sendInput`, `resumeAgent`, `wait`, and `closeAgent` to bounded localized activity labels and exposes only the optional prompt as an 800-character summary. Unknown tools retain a neutral fallback; no second card, state model, timer, network path, or legacy 7420 inference was added.
- Projection smoke covers MCP, Web Search, plan, and collaboration together. CDP browser verification asserts the localized collaboration title and summary inside an expanded completed turn across desktop, phone, and foldable layouts; the focused screenshot is under `output/regression-7420/conversation-collaboration-20260830`.

## Findings: Completed Snapshot Structural Activity Settlement (2026-08-30)

- The fixed Sema transcript treats the turn lifecycle as the completion owner. Installed Windows Codex `26.818.5229` remains the interaction and safety cross-check; neither source supports showing an activity as waiting merely because a refreshed item omits an item-level status field.
- Current App Server `ThreadItem` snapshots omit item status and timing for Web Search, plan, image view, review-mode transitions, and context compaction. CX-Codex previously projected those known structural items as pending even when their authoritative turn was completed.
- The pure projection now settles only status-less `pending` activities inside a completed turn. Explicit `inProgress`, `failed`, and `declined` item facts retain priority, so the fix does not invent completion for an explicitly active or failed operation and adds no legacy 7420 inference path.
- Projection smoke covers all six status-less snapshot item types. CDP verification asserts that the Web Search row says completed after refresh across desktop, phone, and foldable layouts while the existing long-turn, stress, motion, interaction, touch-target, and overflow gates remain green. Evidence is under `output/regression-7420/conversation-structural-settlement-20260830`.

## Findings: Current-schema Dynamic and Image Activities (2026-08-30)

- The 2026-08-28 current-CLI schema audit adds `hookPrompt`, `dynamicToolCall`, and `imageGeneration` to `ThreadItem`. Installed Windows Codex `26.818.5229` routes dynamic tools through its tool-activity adapter and image generation through a distinct presentation, while hook prompts remain internal history context rather than ordinary task prose.
- CX-Codex previously sent all three through the generic unknown fallback, producing unreadable “执行活动” rows and making an internal hook item visible. The fixed Sema source remains the semantic baseline: observable work needs human-readable activity metadata, while internal reasoning or prompt content is not conversation copy.
- The single `ConversationProjection` now omits hook prompts, exposes only a bounded tool label plus namespace for dynamic tools, and exposes only status plus saved path for image generation. Arguments, content items, revised prompts, result payloads, and base64 data do not enter the projection; no second state owner, renderer branch, dependency, or legacy 7420 fallback was added.
- Projection and browser fixtures assert both readability and payload non-disclosure. CDP verification passes at desktop, phone, and coarse-pointer foldable sizes and keeps long-turn, 1600-activity, motion, touch-target, and overflow gates green; independent Headless Playwright repeats the three-viewport readability, non-disclosure, and overflow checks. Evidence is under `output/regression-7420/conversation-current-schema-20260830`.

## Findings: Model Safety Buffering Notification (2026-08-30)

- Windows Codex `26.818.5229` handles `model/safetyBuffering/updated` as turn metadata with `fasterModel`, `reasons`, `showBufferingUi`, and `useCases`; the current live App Server schema also requires the owning `threadId`, `turnId`, and `model`.
- The desktop UI selects the first turn whose safety-buffering metadata requests UI, shows its banner only while that turn is still in progress, and keeps retry/dismiss actions outside the transcript item sequence.
- CX-Codex keeps the fixed Sema projection as transcript authority: this notification may keep the owning turn running but must not create an activity, commentary message, or final answer. Until CX-Codex owns the full discard-and-retry action safely, classify the method as known without adding a partial banner or legacy 7420 inference.

## Findings: Current Official Host-notification Boundary (2026-08-30)

- The 2026-08-28 live App Server schema contains 75 official `ServerNotification` methods. Ten current methods were absent from CX-Codex's known classifier, while `thread/deleted` was already covered by the existing thread-invalidation suffix rule.
- Windows Codex `26.818.5229` recognizes the same current host notifications, but its notification map does not make every official method a conversation item. In particular, `turn/moderationMetadata`, strict auto-approval review, project changes, and several host lifecycle updates remain outside the visible turn item sequence.
- CX-Codex therefore classifies the complete current schema without inventing UI or transcript semantics. The fixed Sema projection remains authoritative: moderation, safety, project/environment/settings, and queue metadata cannot become activities, commentary, or final output merely because the host protocol recognizes them.

## Findings: Latest-first Process Disclosure and Goal Menu (2026-08-31)

- The fixed Sema commit remains the conversation semantic authority. CX-Codex retains every commentary/activity block in the single `ConversationProjection`; the latest-first behavior is only a bounded rendering window and must never become filtering, final inference, cache reconciliation, or a legacy 7420 fallback.
- Running process disclosure stays open but mounts only the latest structured process block by default. When older process exists, the latest body is clamped to three lines; explicit history disclosure loads 17 older blocks per batch, and collapsing returns to the latest-only view. Completed process remains collapsed by default and uses the same latest-first rule after explicit opening.
- The explicit final stays separate from process disclosure and receives the primary reading hierarchy. Process controls remain secondary, keyboard-operable, and at least 44px on phone/coarse-pointer layouts.
- `设置持续目标` is a CX-Codex workflow adaptation inside the Composer `+` menu, not a claim of exact installed Codex menu parity. The existing App Server goal editor and active-goal bar remain the owners; the menu action only closes the attachment surface and opens/focuses that editor.
- Current 7420 Web CDP verification passed the structured conversation and Composer fixtures across desktop, 393 x 852 phone, and 884 x 1104 foldable widths. A 1600-activity turn mounted one process row by default, phone/coarse targets stayed at 44px, the goal menu action opened one focused editor with no standalone create button, and every tested viewport had zero horizontal overflow. Evidence is under `output/regression-7420/latest-process-goal-20260831`.
- Real-session mobile regression exposed an ordering boundary outside the renderer: a newer buffered event sequence caused `applyRuntimeSnapshotState` to reject the entire Runtime snapshot before retaining its structured `thread/read` window, leaving only the newest live turn and hiding historical explicit finals. Structured history is now retained before the Runtime-version gate; only stale execution fields are rejected, and newer notifications still overlay the snapshot by stable turn/item identity inside the same projection.
- After the fix, cold mobile samples from 500ms through 20s retained 10 recent turns and eight explicit finals. The strict real-session gate then passed desktop, 393 x 852 phone, and 884 x 1104 foldable widths with at most one final per turn, zero legacy overlays, zero internal-context leakage, zero horizontal overflow, and zero browser errors; the current live Web entry hash is `013BB44446D03F6F21E517081E41B86D5C0AE93CE4FEBE2FF44CE112FF2254D9`.

## Findings: Projection-owned rollback reconciliation (2026-08-30)

- The installed Windows Codex package `26.818.5229` applies the accepted `thread/rollback` response directly to its structured conversation store, replaces the existing history, and resets pagination and pending requests. It does not wait for a legacy flattened message cache to infer which turns remain.
- CX-Codex now uses the visible Sema-style `ConversationProjection` for rollback counts, worktree-message lookup, and older-history anchors. The authoritative rollback response atomically replaces retained `threadRead` pages, while pre-rollback notifications are cleared before the normal authoritative refresh can reconcile newer facts.
- The Sema-style compact two-step inline confirmation remains the user boundary. A headless Playwright phone check at 393 x 852 changed `回退到此轮` to `确认回退` on the first activation, emitted only on the second activation, reported no page errors, and captured `output/codex-app-parity/sema-rollback-confirmation-phone-20260830.png`.

## Findings: Projection-owned execution recovery (2026-08-30)

- Installed Windows Codex `26.818.5229` derives active command work from structured current-turn items and their `inProgress` status. Its `local-conversation-thread` selector reads `turn.items`, resolves the matching command item by stable ID, and keeps turn lifecycle separate from a flattened transcript cache.
- CX-Codex still allowed old persisted/live `UiMessage.commandExecution` rows to influence execution activity, stop availability, stale recovery, and queue cleanup outside the renderer. Those decisions now consume the latest Sema-style `ConversationProjection` activity plus fresh Runtime/event/request evidence; a terminal projected turn always rejects stale in-progress residue.
- Headless Playwright at 393 x 852 verified one active `sync-degraded` turn, pinned-open process disclosure, exact `正在恢复实时状态` copy, zero page errors, and zero horizontal overflow. Evidence is `output/codex-app-parity/projection-owned-execution-20260830/projection-running-phone.png`.

## Findings: Projection-only realtime state (2026-08-30)

- The running Windows Codex package `26.818.5229` (internal app version `26.818.41509`) derives active commands from the current structured turn's `items`, selecting `commandExecution` items whose `status` is `inProgress`; background cleanup also walks structured turns/items and stable item ids.
- Once realtime notifications are retained for projection, a second per-thread cache of flattened agent messages, plans, raw reasoning text, commands, or activity labels is not a parity requirement. It adds memory and can incorrectly outlive the structured turn lifecycle.
- CX-Codex keeps realtime conversation events only in `conversationNotificationsByThreadId` until authoritative `thread/read` reconciliation. Runtime/outbox/queue/recovery state remains separate infrastructure, while UI, execution labels, stop availability, and task-pet activity consume the projected turn model.

## Findings: Transport feedback is not execution (2026-09-09)

- Installed Windows Codex package `26.901.6511.0` associates execution with structured turn IDs and turn-start events; the queued-message list is a separate queue surface. CX `thread/started` must not imply `turn/started`.
- Keep local sending/confirmation/acceptance visible as `submitting` until execution is established. Do not borrow the previous Runtime generation's completion or elapsed time; terminal turn evidence must not be revived by an older active snapshot.
- `tests.md` documents the state/identity regression commands. `output/send-feedback-browser-20260909` contains isolated desktop/mobile screenshots, not a production or device latency claim.

## Findings: Reply continuity, attachment presentation and visible milestones (2026-09-09)

- Current Windows Codex `26.901.6511.0` recognizes `## My request:` and `## My request for Codex:` after attachment metadata; its user-message renderer separates image URLs/local paths from prose. CX parses only complete leading envelopes in the existing projection and renders local images through its already authorized HTTP route, with a retry state rather than an empty source.
- The fixed Sema ChatView keeps session snapshots as the content owner. A temporarily incomplete CX sidebar list must not prune the currently selected non-hidden conversation; explicit rollback/archive still remove content through their existing owners. This is a state-cleanup correction, not a second retained assistant cache.
- DOM milestones are a CX diagnostic adaptation, not copied desktop internals. Only exact current-page message/turn/item identities enter visibility checks; historical nodes do not consume the bounded observation budget. Completed milestones stop layout/storage work. The legacy `assistantRenderOverhead` key means data-to-visible wall time, not pure rendering CPU time.
- Vue scoped CSS must be checked after compilation: `.dark .transcript-root` retains the component scope, while the former `:global(.dark) .target` spelling dropped the target and leaked global rules. Browser/compiled-style checks cover visible Markdown headings/list markers and scoped dark surfaces.
- Local candidate verification uses the current agent-browser route on a bridge-free static fixture server, plus public projection/state tests. `output/reply-quality-20260909/report.md` separates this evidence from unchanged 7420, model latency, Android and release gates.

## Findings: Native plans, goal races and queue lifecycle (2026-09-10)

- Installed Windows Codex `26.901.6511.0` retains a completed-plan implementation prompt. Its bundled CLI `0.153.4` schema exposes experimental `collaborationMode` with `plan` / `default` and required `settings.model`; top-level `mode` is not a supported turn-start field. CX now translates local modes accordingly without an extra lookup for an explicit model. Model/transport failures cannot authorize another start; only a definite unsupported-field parameter rejection permits legacy Plan fallback.
- Native plan snapshots have text but no item status. A completed authoritative turn must settle retained plan streaming state; progress checklists remain separate from proposal cards. CX derives submission from the next accepted confirmation, scopes identity to the turn/thread projection and does not persist a parallel plan-submission cache.
