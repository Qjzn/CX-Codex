...
function Assert-BoundedRuntimeSendRecoverySource {
  ...
  $messageLoadMatch = [regex]::Match(
    $source,
    "async\s+function\s+loadMessages\([\s\S]*?(?=\r?\n\s*async\s+function\s+loadOlderHistoryForSelectedThread)",
    [System.Text.RegularExpressions.RegexOptions]::Singleline
  )
  Assert-True ($messageLoadMatch.Success) "could not find the selected-thread message loading owner"
  Assert-True ($messageLoadMatch.Value -notmatch "ensureThreadResumed\(") "opening or refreshing a conversation must remain read-only and must not claim the App Server writer"
...
function Assert-ConversationNativeWriterQueue {
  param([string]$Session)

  $metrics = Invoke-BrowserEvalJson -Session $Session -Script @'
JSON.stringify((() => {
  const queue = document.querySelector('.conversation-regression-queue');
  const caption = queue?.querySelector('.queued-messages-caption')?.textContent?.replace(/\s+/g, ' ').trim() || '';
  const rows = Array.from(queue?.querySelectorAll('.queued-row') || []);
  return {
    queueCount: rows.length,
    caption,
    rowTexts: rows.map((row) => row.querySelector('.queued-row-text')?.textContent?.replace(/\s+/g, ' ').trim() || ''),
    retryCount: queue?.querySelectorAll('.queued-row-retry').length || 0,
    deleteCount: queue?.querySelectorAll('.queued-row-delete').length || 0,
    loadingCount: queue?.querySelectorAll('.loading-inline').length || 0,
    hasHorizontalOverflow: document.documentElement.scrollWidth > document.documentElement.clientWidth + 2
  };
})())
'@
  Assert-True ([int]$metrics.queueCount -eq 2) "native-writer handoff must keep both queued messages visible"
  Assert-True ([string]$metrics.caption -eq '已交给正在使用此任务的 Codex；当前回复结束后会自动继续。') "native-writer handoff must explain automatic continuation without presenting a send failure"
  Assert-True ([int]$metrics.retryCount -eq 0) "accepted native-writer queue rows must not be mislabeled as failed retries"
  Assert-True ([int]$metrics.deleteCount -eq 2) "accepted native-writer queue rows must remain individually removable"
  Assert-True ([int]$metrics.loadingCount -eq 0) "accepted native-writer queue ownership must not remain in a false syncing state"
