...
- M2 状态绝对正确：执行中。在不依赖 GitHub 外部授权的本地链路上，已修复事件流代际重置、App Server 传输结果不确定、双页面 outbox 覆盖、旧轮次终态清除当前轮次，以及服务进程冷启动丢失已接受 `pending_start` 的问题。浏览器按消息日志合并并用删除墓碑防止旧页面复活消息，日志按消息压缩并设置硬上限；服务端仅自动恢复正文哈希一致且尚未进入 `starting` 的请求，进入 RPC 前即清除临时正文，避免重复发送和长期保留。消息签名、持久 `clientMessageId`、重放分页、序列缺口、排队接管与 SQLite 重启故障注入矩阵已通过。Android 原生现在保存精确请求 turn 代际，并以 Runtime 权威快照和开始/完成时间栅栏结算；同一代活动任务不会再被前端瞬时终态提前结束。OPPO Android 16 真机已通过锁屏、强制 Doze、10 轮网络切换、应用进程回收、通知点击与去重，最近任务划除后的 OEM 自启动仍阻塞。
...
- 外部写入已获本次目标授权：可以在本地与真机门槛通过后提交、推送、创建/合并 PR，并在主分支和 Release 工作流均通过后发布 v2.8.0。当前尚未执行这些动作；仓库规则仍禁止把本地验证包、调试 APK 或脏工作树证据描述为已发布版本。
...
- 需要设备或 OEM 授权：OPPO Android 16 已完成进程回收、锁屏、Doze、10 轮网络切换、通知点击/去重；最近任务划除会终止整个包并取消 Alarm，仍需系统“允许后台运行/自启动”授权或已配置的 FCM 权威唤醒后复测。410×502 Android 9 实体手表、最终签名 Release APK、真实 Windows 高对比度和屏幕阅读器仍需独立系统级验证。代码、JVM、lint、浏览器仿真或截图不能替代这些证据。
