...
首次源码预览安装最终成功用时 153.5 秒。正式 2.5.6 已完成公开资产与安装回归，并补上仅限本机的二维码配对页。正式 2.5.7 修复了外层 PowerShell 捕获安装 JSON 时调用脚本不退出的问题；后续公开资产回归又发现安装器可能在公网验证仍为 `verifying` 时提前返回 `ok=false`，正式 2.5.8 已改为等待运行时真正 `ready`，并完成最新公开资产复测。项目仍不能称为真正“傻瓜式”，主要差距变为：下载阶段缺少连续进度、首次工作区选择不够明确，以及手机扫码后仍需手动输入密码。
...
| 浏览器接收 | 收到“新人安装验证成功” |
| 实时连接 | 正常 |
| 正式 Release 首次安装 | 2.5.3 建立基线；最新 2.5.8 公开资产一步安装通过 |
