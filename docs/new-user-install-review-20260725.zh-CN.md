...
首次源码预览安装最终成功用时 153.5 秒。随后从 GitHub 正式 2.5.1 Release 完成无现有程序的安装，SHA-256、本机健康和三项 Quick Tunnel 鉴权验证均通过；2.5.2 正式升级回归继续发现 Codex App Server 子进程持有目录的问题，2.5.3 已改为停止受管进程树、有限重试并在失败后恢复旧服务。当前仍不能称为真正“傻瓜式”，主要差距变为：下载阶段缺少连续进度、首次工作区选择和二维码配对仍不够明确。
...
| 浏览器接收 | 收到“新人安装验证成功” |
| 实时连接 | 正常 |
| 正式 Release 首次安装 | 2.5.1 通过 |
| 运行中重复升级 | 2.5.3 进程树修复并纳入 Windows CI |
...
- 2.5.3 补齐运行中重复升级，Windows CI 会让旧服务及其子进程锁定目录后再次执行 bootstrap。
...
1. 已完成：2.5.1 正式 Release 已完成公网下载与首次安装验证；2.5.3 继续关闭运行中升级的父子进程与失败恢复边界。
