...
- 发送后立即保留消息；弱网或确认延迟时减少误报失败、重复消息和队列状态跳动。
- 默认展示公开进展、运行动画和本轮耗时；命令与输出按需展开，阅读历史不被新内容打断。
- 修复跨轮计时串用、旧快照覆盖及前台恢复漏读，保留已完成结果。
- 加固计划、目标与队列的一致性，合入依赖安全与 Windows 启动器修复。

仍为未发布候选；当前版本的部署、CI、设备与发布门槛分别记录，不能互相替代。

## 2.8.0 - 2026-08-30

- 侧栏快捷入口进一步精简：顶部集中侧栏开关、全部已读和新会话，下方固定显示搜索、技能、GitHub；移除工作台、诊断前端页面及工具二级菜单。移动端设置不再常驻展示原生网络、设备状态和后台运行三条状态，底层诊断与后台恢复能力保持不变。
- 助手回复中的 Markdown 标题、段落、无序列表、有序列表、引用、代码块、表格和分隔线现在按语义结构呈现；`###` 与 `-` 不再作为普通字符直接显示，长对话仍保持紧凑、可扫描的 Codex 风格层级。
- 上传图片或文件时，Codex 宿主附加的 `Files mentioned by the user`、附件路径清单和 `My request` 传输信封不再混入用户消息正文；真实问题与结构化附件卡片继续保留，普通文本里同名标题不会被误删。
- Android 从后台返回并取得权威快照后会立即结束恢复反馈和计时，即使该快照因事件序列较旧而不能覆盖最新状态；旧版错误计量样本会自动失效，避免设置页被不真实的超长恢复耗时污染。
- 本地候选版本提升为 2.8.0，避免继续以公开 v2.7.6 的版本号承载未发布改动；前端构建标识改为稳定包名，Tailwind 只扫描真实 `src` 源码，不再受仓库说明、测试证据、`node_modules` 或临时安装目录名称影响。发布安装器在不同暂存目录重建时应生成相同前端资产，设置页也不会再把一次性缓存目录误显示为工作区。
- H5 首页回归现在会把模型面板逐项对照实时 `model/list`，检查分页结果、显示名、描述、默认标记、可用状态和唯一选中项，避免只靠静态 Composer 夹具掩盖模型元数据加载退化。
- 当前发布宣传与验收统一以产品目标、当前 App Server 协议矩阵和当前 schema 审计摘要为事实源；2026-07-05 的 Candidate review / PR pack 仅保留作历史追溯，不再被 Release 指南或模板当作当前状态。
- 修复归档会话列表已有旧缓存且后台刷新尚未完成时，后续读取仍等待慢速 App Server 扫描的问题；现在所有读者立即复用旧缓存，同一键只保留一个后台刷新，刷新完成后再无感替换权威结果。
- Windows/Release 打包白名单现包含 README 已公开链接的 `PRODUCT_GOAL.md`，release package smoke 会强制检查该文件，避免本地更新或正式压缩包中的“产品目标与完成门槛”入口断链。
- Skills 私有 Git 同步不再把 GitHub 令牌拼进命令参数或远程 URL，也不会把带凭据地址持久化到 `.git/config`。认证只通过单次 Git 网络子进程的环境变量和临时 credential helper 提供，自动命令禁用 hooks；拉取拆分为认证 fetch 与无凭据 merge，已有远程地址会在联网前重写为干净的 GitHub URL。Git 与 `rg` 从绝对 `PATH` 项解析，Skills、Worktree、Rollback 和工作区文件枚举不会执行用户工作树里的同名程序。Windows 同步状态改用系统 DPAPI `CurrentUser` 加密，旧明文状态首次读取时原地迁移；Linux/macOS 文件在写入前后强制为 `0600`。公开回归夹具和历史测试证据中的本机用户名、真实工作目录与个人简历文件名也已替换为稳定示例，并加入治理防回退断言。
- 两小时稳定性浸泡改为零容忍门禁：除 HTTP 可达外还逐样本验证 JSON 健康状态、App Server 运行/初始化/非停止、服务端待处理请求、Runtime 未确定请求、活动 plan-mode turn 与慢 `thread/list`；App Server PID 和 Runtime 事件流代际必须全程稳定，健康与回放端点必须属于同一事件流。回环隔离运行会把公网健康和鉴权同时明确标记为未检查，不再把跳过检查显示为通过。
- 使用当前本机 `codex-cli 0.130.0` 刷新 App Server schema 脱敏审计，并让 release verifier 以 warn 模式完成复核；四组 added/removed 完整集合与 2026-07-04 快照逐项相同，没有把既有 drift 误报为新回归。旧 Release Readiness 文档已标记为历史快照，当前状态统一指向产品目标、项目规格和测试证据。
- 主内容区现在暴露唯一且命名的 `main` 地标；桌面侧栏与移动抽屉统一播报“会话导航”，宽度调节器使用中文可访问名称。现有“跳到主要内容”和焦点恢复行为保持不变。
- 升级 DOMPurify 至 3.4.13、PDF.js 至 6.2.108，并锁定 brace-expansion、esbuild、nanoid 与 PostCSS 的已修复版本；新增完整锁文件安全审计门禁并接入 CI/Release。PDF 仍只走 legacy Canvas 渲染，本地预览页新增 header 与 meta 双层 CSP，不启用注释脚本管理器。
- App Server 新安装默认从无需审批的全磁盘权限收紧为 `on-request + workspace-write`，与 Codex 桌面端的 Auto 权限边界一致；原有环境变量仍可显式选择其他官方策略，非法值回退到安全默认值。
- 本地图片、文件打开、目录浏览和文本编辑统一限制在已登记的工作区根目录内；真实路径复核会拒绝相邻前缀目录、符号链接和 Windows junction 逃逸，工作区内隐藏文件仍可正常使用。
- CLI 不再允许把 `--no-password` 与通配、局域网或其他非回环绑定组合；无密码模式仅保留给 localhost / 回环地址的本机调试，拒绝发生在端口监听之前。
- 详细健康与诊断响应增加最终统一脱敏层：嵌套的密码、Authorization、Cookie、secret、API key、access/auth/refresh token 字段和常见敏感错误片段不再依赖各数据源自行清理。
- 事件存储重置或更换后，前端会清除旧事件代际的快照排序水位；恢复期间混入的旧流实时事件不会再形成重复恢复循环，新流中真正已持久化的事件会通过一次复核重放补回。
- `turn/start` 与 `turn/interrupt` 遇到 App Server 超时、进程退出或传输重启时统一进入结果不确定状态并等待权威对账，不再把可能已经生效的操作误报为确定失败。
- 多个浏览器页面离线发送时，outbox 改用逐消息写前日志和删除墓碑合并；后写页面不再覆盖另一页面的待发送消息，旧页面也不能在删除后把消息复活。日志只保留每条消息最新版本，并分别限制为最多 12 条待发送记录和 256 条删除墓碑，避免长期多页面使用形成无界本地存储。
- 旧轮次的完成、失败或中断事件只更新自身序列水位，不再清除当前新轮次、消息队列或 outbox；终态副作用必须与当前活动 `turnId` 匹配。
- 7420 服务在接受消息后、调用 App Server 前意外重启时，会从 SQLite 恢复仍为 `pending_start` 且正文哈希一致的原请求；`requestId/clientMessageId` 保持不变，调用方未提供消息标识时统一复用 `requestId`，进入 `starting` 前即把临时正文替换为摘要，已经开始或结果不确定的请求绝不自动重发。
- 7420 浏览器回归把已有会话与新会话的本地气泡/运行反馈收紧到 100 ms 内，并把 1600 条消息流式场景收紧到主线程心跳延迟低于 80 ms、实际挂载消息节点不超过 20 个。任务首屏现在按每次选择独立计时并区分本地缓存、内存与网络来源，冷重载的本地缓存首屏必须在 300 ms 内；前台恢复只在通过序列校验的权威快照到达时结算，滚动保留最近 7 天最多 50 个无正文样本，连续 5 次恢复及 P95 必须在 2 秒内。
- README 将 Windows 一条命令安装、2–5 分钟预期、默认本机入口和失败后的最小诊断提前到首屏，常用入口只保留 Release、Android、远程访问、更新、安全和产品目标；协议审计与候选发布材料继续保留在文档索引，不再阻断新用户主路径。
- `PROJECT_SPEC.md` 从旧版功能愿望清单收敛为当前架构事实：更新 Node/Vue/Vite/Commander/Capacitor 版本，记录 Runtime Store、消息所有权、事件重放、安全默认值和关键模块，并移除把已实现 fork、rollback、skills、计划模式、token usage 与工具流式更新列为“未实现”的矛盾说明。
...
- Android 回到前台时改为按待处理、未读、连接陈旧或内容未加载等证据触发延迟重试，避免每次切换应用都盲目重复完整刷新；WebView 无响应、被系统回收或崩溃时显示明确恢复状态并自动重建页面。
- 保持 Android WebView 默认渲染器优先级，不再在后台主动降低其存活优先级；低内存回收恢复原会话路由，真实渲染崩溃则回到安全入口，避免白屏和崩溃循环。
- Windows 启动 Codex App Server 子进程时隐藏后台控制台窗口，减少服务运行期间的弹窗干扰；感谢社区贡献者 [@534A4D21](https://github.com/534A4D21)。
- 修复外部 fork 拉取请求的 Windows bootstrap 烟测取错仓库或分支的问题，同时保留活动任务升级夹具，避免社区贡献被无关 CI 失败阻塞。

## 2.7.4 - 2026-08-04

- 修复 2.7.3 Release ZIP 漏打 `vite.local-preview.config.ts`，导致 Windows 全新安装在本地预览构建阶段失败的问题；2.7.3 已撤回稳定最新版身份，由 2.7.4 替代。
- 将主 Vite 配置和本地预览 Vite 配置同时纳入发布包必需清单、ZIP 内容断言和治理检查，缺少任一构建入口都会在发布前失败。
- 延续 2.7.3 已完成验证的会话队列隔离、精确重排、长会话增量读取、计划与持续目标、文件引用、Android PDF 与任务宠物稳定性修复。

## 2.7.3 - 2026-08-04

- 计划模式改为持续生效，并新增结构化计划卡、一步提交实施和线程级持续目标；目标的暂停、继续、编辑、清除、预算与耗时都以 App Server 状态为准。
- 消息排队改由 7420 服务端持久接管：切换会话、刷新页面或关闭 Android 页面后仍会按顺序发送；修复排队消息被旧完成快照误判成功、并入上一轮或静默丢失的问题。
- 快速连续排队、删除、重试和失败阻塞改为按稳定消息 ID 合并；完整同步后才执行服务端精确重排，遇到并发变化会回退到权威顺序，不再由旧异步写入覆盖新队列。
- 长会话优先读取最近 40 轮的本地有界投影，连续追加只读取新增字节；静默 1.8 秒后再做一次权威收敛，既减少 6–8 秒重读，也保留计划、命令、文件、审批和工具记录。
- 已发送但尚未进入权威历史的用户消息可跨刷新恢复；外部 Codex 进程写入的 commentary 会及时显示，不再被误判为最终回复或触发高频完整会话读取。
- Codex 文件引用会显示为可打开的真实文件名，Windows/Unicode 路径与附加属性安全保留；Android 旧版 WebView 改用 PDF.js legacy 主线程和 worker，PDF 预览不再因缺少新 JavaScript API 而空白。
- Android 任务宠物改为安静贴边模式：默认只保留侧边入口，新助手回复短暂显示五秒，任务列表八秒自动收起，避免长期遮挡桌面。
- 未读提醒改用持久化回复事件游标；进入对应会话后立即标记已读，普通执行中任务不再被误计为未读，也不会持续显示“工作中”。
- 浮窗任务列表收敛为最多三条并继续直达对应会话；移除最近会话和浮窗输入框，应用位于前台时不重复展示浮窗或完成通知。

## 2.7.2 - 2026-08-02

- 左侧会话区收敛为“置顶会话 + 最近活动目录 + 目录内最近会话”的固定层级，目录和会话都随最近活动自然上移；新增全部收起/展开和“定位当前会话”，不再暴露与最近排序冲突的手动置顶目录或排序入口。
- 会话操作统一到一个不会被侧栏裁切的菜单，支持完整会话复制、Markdown 导出、复制同站会话链接、标记未读、置顶、重命名、分叉和归档；归档后可直接撤销，点击菜单外部会可靠关闭。
- `Ctrl/Command + P` 新增当前工作区文件快速打开，文件枚举具备短时复用、并发合并、超时和输出上限；侧栏全量搜索新增可恢复索引缓存、渐进结果和输入连续性，冷启动时不会长时间阻塞列表。
- 优化长会话与移动端使用：手机默认 Enter 换行、桌面默认 Enter 发送，输入框自动增高受限；等待确认的任务优先显示，图片缩放围绕手势锚点，前后台切换时保留阅读位置并只恢复必要同步。
- 会话列表缓存改为异步合并写入，session-index 与 session-log 变更按来源刷新，已完成轮次以权威快照清理旧投影，降低列表刷新、长会话切换和恢复场景中的卡顿与重复内容。
- Quick Tunnel 再次启动时会重新验证公网健康、HTTP 鉴权和 WebSocket 鉴权；子进程仍存活但临时域名或公网连接已失效时，不再复用错误的 `ready` 状态，而会关闭旧通道并建立新地址。
