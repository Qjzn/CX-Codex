...
首次源码预览安装最终成功用时 153.5 秒。随后又从 GitHub 正式 2.5.1 Release 完成无现有程序的安装，SHA-256、本机健康和三项 Quick Tunnel 鉴权验证均通过；重复执行同一命令时进一步发现并修复了 Windows 运行目录锁定问题，该修复进入 2.5.2。当前仍不能称为真正“傻瓜式”，主要差距变为：下载阶段缺少连续进度、首次工作区选择和二维码配对仍不够明确。
...
| 运行中重复升级 | 2.5.2 修复并纳入 Windows CI |
...
11. 正式 2.5.1 首次安装成功后，重复执行原命令会因旧服务占用安装目录而无法原子替换。2.5.2 在替换前精确停止目标服务与隧道，并新增“旧目录被进程占用”的原地升级门禁。
...
- 2.5.2 补齐运行中重复升级，Windows CI 会锁定旧安装目录后再次执行 bootstrap。
...
1. 已完成：2.5.1 正式 Release 已完成公网下载与首次安装验证；2.5.2 继续修复运行中升级。
