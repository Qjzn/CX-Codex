...
- Local Runtime summaries bind start/end times to the execution generation: a new turn or new accepted execution clears the previous clock, while same-turn waiting/sync recovery preserves it. Native Unix-second lifecycle fields remain numeric until normalization; absent start evidence does not create a `Date.now()` execution start.
- User display identity is separate from authoritative item/turn IDs. Runtime start forwards the durable `clientMessageId` as native `clientUserMessageId`; snapshot/live user items read native `clientId` first, with older aliases retained as fallbacks. One-to-one same-turn native event/history aliases can reconcile a live UUID with a history item ID; ambiguous or cross-turn claims remain separate. Confirmed display bindings live in the existing bounded user cache before outbox cleanup. A returned turn ID is not first-user ownership: modern `turn/start` can also steer an active turn. No custom opener proof is generated or interpreted. This local candidate requires matched Web/CLI code; legacy session-log recovery and cross-component handoff limits remain recorded in `tests.md`.
- Session-log fallback preserves a user event's own `client_id` as `clientId`, local image paths, source and timestamp through incremental reads. Only an observed, non-conflicting task lifecycle supplies its turn; byte offsets identify log records independently of read chunking. File identity and bounded prefix/tail checks guard cache reuse. There is no proven alias from the event to a raw response item, so identified events bypass body-only deduplication; unmatched representations remain separate.
- Explicit session-log `task_started`, `task_complete` and `turn_aborted` now recover matching turn state and lifecycle clocks. Native Unix-second fields take precedence; enclosing event time is the older-record compatibility fallback. Explicit null clocks stay unknown through projection instead of borrowing item/polling time. Duplicate settled starts cannot revive a turn or block the next one; missing/foreign terminal IDs never settle the last visible turn. Old logs without lifecycle retain compatibility. Empty recovery boundaries remain private and are counted only when their first visible item arrives. Native wall duration is preserved but is not assumed to exclude user waiting.
- Public progress and technical details have separate bounded presentation windows. The latest two commentary entries are visible while following; operations mount on demand. Selection, process focus and scrolling away protect the mounted text and prevent automatic collapse, while explicit message navigation refreshes its target. A local display-only clock runs independently of snapshot arrival; it never generates task state, polls an API or invents an unknown duration. This is a local candidate, not proof of production or device latency.
- Explicit conversation refresh and content-invalidating recovery bypass the 12-second quiet-detail throttle and the gateway's 900ms snapshot cache. If an older read is in flight, a trailing state read serves concurrent refresh waiters; only successfully processed refresh reads satisfy them, not a cancelled or cache-only read. Runtime-only status/reconcile responses cannot declare body availability: same-version status preserves the known body source, newer lifecycle/sequence or stale status downgrades retained fresh content to cached, and actual state-read unavailability remains visible. These are read-side coordination rules, not another conversation store or a reason to resume the native writer.
- Session-log history source is probed independently of the 24 MB body window. Only a complete first nonempty `session_meta` within a 64 KiB CX budget, with a matching thread ID and explicit `history_mode: legacy`, enables native UI-only user recovery from the first read. Ordinary model-input users are then excluded from recovered UI history even before an event arrives. A complete or trusted incremental empty history remains usable without heavy RPC fallback; an empty cold truncated tail does not. The full head evidence joins the file-identity/checkpoint guard, and the cache is also thread-ID scoped. Native-source reads whose pre/post evidence changes are rejected before outer caching; private cloned recovery turns retain hidden boundaries without exposing empty UI turns. Missing, unknown, oversized, incomplete or mismatched heads retain conservative compatibility; later copied metadata cannot upgrade that decision. This intentionally differs from native missing-mode defaults and unknown-mode rejection. Raw files are unchanged; paginated completed-item recovery remains unimplemented.
- Visible user source is also selected per turn: when a recovered UI user event or native snapshot/notification user exists, remaining compatibility model-input users tagged `recoverySource: response_item` do not become transcript users. This follows Codex 0.153.4 `ThreadHistoryBuilder` source semantics, not text deduplication or an inferred client alias. Unknown response-only history remains readable without acknowledging a local request. Every projected user block is rendered under its stable display key, including same-turn follow-ups; only the opener owns turn rollback. Unknown-format raw-only intervals and cross-component handoff remain separate compatibility work.
