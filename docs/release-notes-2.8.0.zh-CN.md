...
- 侧栏主路径更直接：顶部保留侧栏开关、全部已读和新会话，下方固定显示搜索、技能与 GitHub，不再经过工具二级菜单；低频工作台与诊断前端页面已经移除，后台执行和服务端诊断能力不受影响。
- 连接设置不再常驻展示原生网络、设备状态和后台运行三条技术状态，减少普通使用中的信息噪音；深度休眠通知和后台运行调整入口继续保留。
