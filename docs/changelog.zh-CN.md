...
- 对话增加 Mermaid 图表和图片访问处理，当前任务增加侧边对话、终端及工作区入口。
- 改进 tunnel 网页登录、认证与本地文件访问边界；侧边终端的 WebSocket 连接增加同源校验。
- Android 壳补充服务地址配置；相关前端、服务模块和移动端回归随版本更新。
- 发布说明见 [2.8.1](./release-notes-2.8.1.zh-CN.md)。
