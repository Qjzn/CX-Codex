...
首次源码预览安装最终成功用时 153.5 秒。正式 2.5.3 已从公开资产完成运行中升级、同版本重复升级、默认保留数据卸载和无安装状态重装，本机健康和三项 Quick Tunnel 鉴权验证均通过；正式卸载暴露的超时误警告已在 2.5.4 候选中改为子进程优先停止整棵受管进程树。当前仍不能称为真正“傻瓜式”，主要差距变为：下载阶段缺少连续进度、首次工作区选择和二维码配对仍不够明确。
...
| 默认保留数据卸载与重装 | 2.5.3 通过；2.5.4 候选关闭超时误警告 |
...
- 2.5.4 候选让官方卸载器以子进程优先顺序关闭整棵受管进程树，并用真实监听根进程与子进程要求无超时警告。
...
1. 已完成：2.5.3 正式 Release 已完成公网下载、运行中升级、同版本重复升级、卸载和重装验证；2.5.4 候选继续关闭卸载的父子进程与误警告边界。
