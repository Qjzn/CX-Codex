...
| 审批与权限 | 新增 permission profile、guardian review、network/file-system permission、approval review 通知 | 部分覆盖。当前已有 command/file/mcp 基础审批和停止审计，诊断中心已脱敏展示 pending server request 队列，但未覆盖 guardian/profile | 先做只读展示和拒绝/降级策略，避免默认放权；所有新增 approval 必须记录来源、scope、decision |
