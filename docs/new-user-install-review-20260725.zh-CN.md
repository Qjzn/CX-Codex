...
首次源码预览安装最终成功用时 153.5 秒。正式 2.5.6 已完成公开资产与安装回归，并补上仅限本机的二维码配对页。正式 2.5.7 修复了外层 PowerShell 捕获安装 JSON 时调用脚本不退出的问题；后续公开资产回归又发现安装器可能在公网验证仍为 `verifying` 时提前返回 `ok=false`，2.5.8 候选已改为等待运行时真正 `ready`。项目仍不能称为真正“傻瓜式”，主要差距变为：下载阶段缺少连续进度、首次工作区选择不够明确，以及手机扫码后仍需手动输入密码。
...
| 安装结果等待公网就绪 | 2.5.8 候选已在真实 7420 源码安装中等待到 `ready` 后返回，三项公网验证全部通过 |
...
13. 正式 2.5.7 公开资产首次安装中，本机健康已通过、Quick Tunnel 几秒后也完全可用，但安装器在运行时仍为 `verifying` 时读取了一次状态并返回 `ok=false`。2.5.8 候选会持续等待 `active + ready + health + HTTP auth + WebSocket auth`，并给瞬时公网失败重试保留约 4 秒窗口。
...
- 2.5.7 发布后公开安装实测发现并复现了 `verifying` 提前返回；2.5.8 候选源码已在同一台机器以外层变量捕获方式通过真正的 ready 边界。
